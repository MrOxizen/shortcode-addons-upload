...
class Style_2 extends Templates
{
    ...
    public function default_render($style, $child, $admin)
    {
        ...
        foreach ($all_data as $key => $value) {
            ...
            echo $link;
            echo '<div class="sa_addons_tooltip_container_style_2">
                        <div class="sa_addons_tooltip_style_2 sa_tooltip_unique_' . $key . ' ' . $value['sa_tooltip_posi'] . '">
                            ' . $img . '
                            ' . $tooltip . '
                        </div>
                    </div>';
...
<div class="oxi-addons-container ">
    <div class="oxi-addons-row">
        ...
        foreach ($listdata as $one_item) {
            $heading = $img = $tooltiptext = '';
            $listfiles = explode('||#||', $one_item['files']);
            if (!empty($listfiles[1])) {
                $img = '<img src="' . OxiAddonsUrlConvert($listfiles[1]) . '">';
            }
	...
	.oxi-addons-vr-' . $oxiid . ' .oxi-addons-tt-img-section{
		width :100%;
		float :left;
