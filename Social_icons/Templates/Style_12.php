...
class Style_12 extends Templates {
    ...
    public function default_render($style, $child, $admin) {
        ...
        foreach ($all_data as $key => $listitemdata) {
            ...
            echo '<div class = "oxi-addons-social-style-12  oxi-addons-social-style-12-' . $key . '"  ' . $this->animation_render('sa_social_icons_animation', $style) . '>
                        <a ' . $link . ' class = "oxi-icon-style-12 oxi-icon-item">' . $icon . '</a>
                    </div>';
        }
        echo '</div>';
    }
    ...
    public function old_render() {
        ...
        foreach ($listdata as $value) {
            ...
            if ($styledata[63] == '') {
                $css .= '.oxi-addons-container .oxi-icon-item-' . $value['id'] . '.oxi-icon-' . $oxiid . ':hover:after{
        ...
        $css .= '.oxi-addons-social-' . $oxiid . '{
                ...
                .oxi-addons-container .oxi-icon-' . $oxiid . ':hover:after{
                    ...
                    top: ' . $styledata[67] . 'px;
                    left: ' . $styledata[67] . 'px;
                    width: calc(100% - ' . ($styledata[67] * 2) . 'px);
                    height: calc(100% - ' . ($styledata[67] * 2) . 'px);
                    padding: 0px;
                }
                .oxi-addons-social-' . $oxiid . ' .oxi-icon-' . $oxiid . ' .oxi-icons{
                  z-index: 2;
                ...
                @media only screen and (min-width : 669px) and (max-width : 993px){
                        ...
                        .oxi-addons-container .oxi-icon-' . $oxiid . ':hover:after{
                            top: ' . $styledata[68] . 'px;
                            left: ' . $styledata[68] . 'px;
                            width: calc(100% - ' . ($styledata[68] * 2) . 'px);
                            height: calc(100% - ' . ($styledata[68] * 2) . 'px);
                        }
                        .oxi-addons-social-' . $oxiid . ' .oxi-icon-' . $oxiid . ':hover{
                ...
                @media only screen and (max-width : 668px){ 
                        ...
                        .oxi-addons-container .oxi-icon-' . $oxiid . ':hover:after{
                            top: ' . $styledata[69] . 'px;
                            left: ' . $styledata[69] . 'px;
                            width: calc(100% - ' . ($styledata[69] * 2) . 'px);
                            height: calc(100% - ' . ($styledata[69] * 2) . 'px);
                        }
                        .oxi-addons-social-' . $oxiid . ' .oxi-icon-' . $oxiid . ':hover{
