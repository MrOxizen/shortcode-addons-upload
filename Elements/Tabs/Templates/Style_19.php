...
class Style_19 extends Templates
{
    ...
    public function default_render($style, $child, $admin)
    {
        ...
        echo '<div class="sa-addons-tabs-main-wrapper-style-19" ' . $this->animation_render('sa_tabs_tab_anim', $style) . '>
        ...
        foreach ($child as $header) {
            ...
            if(array_key_exists('sa_tabs_color', $value_header) && $value_header['sa_tabs_color'] != ''):
                $this->CSSDATA .= '.' . $this->WRAPPER . ' .sa-addons-header.sa-header-' . $header['id'] . ' {
                    background: ' . $value_header['sa_tabs_color'] . ' !important;
                }
        ...
        foreach ($child as $body) {
            ...
            if(array_key_exists('sa_tabs_color', $value_body) && $value_body['sa_tabs_color'] != ''):
                $this->CSSDATA .= '.' . $this->WRAPPER . ' .sa-header-' . $header['id'] . ' {
                    background: ' . $value_body['sa_tabs_color'] . ' !important;
                }
                .' . $this->WRAPPER . ' .sa-header-' . $body['id'] . '.sa-active{
                    background: ' . $value_body['sa_tabs_color'] . ' !important;
                }
                .' . $this->WRAPPER . ' #sa-tab-' . $this->oxiid . '-id-' . $body['id'] . '{
                    background: ' . $value_body['sa_tabs_color'] . ' !important;
                }
                ';
            endif;
    ...
    public function inline_public_jquery()
    {
        ...
        $jquery .= ' 
            jQuery(".sa-addons-tabs-main-wrapper-style-19 .sa-addons-header:eq(' . $styledata['sa_tabs_initial'] . ')").addClass("sa-active");
            jQuery(".sa-addons-tabs-main-wrapper-style-19 .sa-addons-header-two:eq(' . $styledata['sa_tabs_initial'] . ')").addClass("sa-active");
            jQuery(".sa-addons-tabs-main-wrapper-style-19 .sa-addons-body:eq(' . $styledata['sa_tabs_initial'] . ')").' . $animationIn . '("slow");
            jQuery(".sa-addons-tabs-main-wrapper-style-19 .sa-addons-header").click(function() {
            if (jQuery(this).hasClass("sa-active")) {
                return false
            } else {
                jQuery(".sa-addons-tabs-main-wrapper-style-19 .sa-addons-header").removeClass("sa-active");
                jQuery(this).addClass("sa-active");
                jQuery(".sa-addons-tabs-main-wrapper-style-19 .sa-addons-body").' . $animationOut . '("slow");
                var activeTab = jQuery(this).attr("ref");
                jQuery(activeTab).' . $animationIn . '("slow"); 
            }
        });
        jQuery(".sa-addons-tabs-main-wrapper-style-19 .sa-addons-header-two").click(function() {
            if (jQuery(this).hasClass("sa-active")) {
                return false
            } else {
                jQuery(".sa-addons-tabs-main-wrapper-style-19 .sa-addons-header-two").removeClass("sa-active");
                jQuery(this).addClass("sa-active");
                jQuery(".sa-addons-tabs-main-wrapper-style-19 .sa-addons-body").' . $animationOut . '("slow");
                var activeTab = jQuery(this).attr("ref");
                jQuery(activeTab).' . $animationIn . '("slow");
        ...
        if (array_key_exists('sa_tabs_tab_fix_header', $styledata) && $styledata['sa_tabs_tab_fix_header'] != '0') {
            $jquery .= 'if(fullwidth <= 668){
                            jQuery("html, body").animate({
                                scrollTop: jQuery(".sa-addons-tabs-main-wrapper-style-19").offset().top - ' . $styledata['sa_tabs_tab_fix_h_offset'] . '
                            }, 2000);
                        } ';
        }
        $jquery .= '}
    ...
    public function old_render()
    {
        ...
        foreach ($listdata as $header) {
            ...
            $css .= '
                .oxi-header-' . $header['id'] . '{
                   background: ' . $value_header[7] . ' !important;
                }
        ...
        foreach ($listdata as $body) {
            ...
            $css .= ' 
                .oxi-header-' . $header['id'] . '{
                   background: ' . $value_header[7] . ';
                }
                .oxi-header-' . $body['id'] . '.oxi-active{
                     background: ' . $value_body[7] . ';
                }
                #oxi-tab-' . $oxiid . '-id-' . $body['id'] . '{
                    background: ' . $value_body[7] . ';
                }
