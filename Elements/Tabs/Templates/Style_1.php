...
class Style_1 extends Templates
{
    ...
    public function default_render($style, $child, $admin)
    {
        ...
        foreach ($child as $header) {
            $value_header = json_decode($header['rawdata'], true);
            $icon = '';
            ...
            if (array_key_exists('sa_tabs_tab_icon_on_off', $value_header)) :
                ...
                $icon_text = $this->text_render($value_header['sa_tabs_h_text']);
            endif;
        ...
        foreach ($child as $body) {
            $value_body = json_decode($body['rawdata'], true);
            ...
            if (array_key_exists('sa_tabs_tab_icon_on_off', $value_body)) :
                ...
                $icon_text = $this->text_render($value_body['sa_tabs_h_text']);
            endif;


            echo '<div class="sa-addons-header-two sa-header-' . $body['id'] . ' " ref="#sa-tab-' . $this->oxiid . '-id-' . $body['id'] . '">' . $icon_text . '</div>';
            echo '<div class="sa-addons-body ' . ($admin == 'admin' ? 'oxi-addons-admin-edit-list ' : '') . '" id="sa-tab-' . $this->oxiid . '-id-' . $body['id'] . '" style="display: none;">' . $this->text_render($value_body['sa_tabs_content']) . '';
            if ($admin == 'admin') :
    ...
    public function inline_public_jquery()
    {
        ...
        if (array_key_exists('sa_el_initial_open', $styledata)) {
            if ($styledata['sa_tabs_tab_fix_header'] == 'fix_header') {
