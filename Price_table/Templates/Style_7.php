...
class Style_7 extends Templates
{
    ...
    public function default_render($style, $child, $admin)
    {
        foreach ($child as $v) {
            ...
            echo '<div class="oxi-addons-parent-wrapper-style-7 ' . ($admin == "admin" ? 'oxi-addons-admin-edit-list' : '') . ' ' . $this->column_render('sa_price_table_column', $style) . '">
                     <div class="oxi-addons-wrapper-style-7" ' . $this->animation_render('sa_price_table_animation', $style) . ' >
                    ' . $ribbon . '
    ...
    public function old_render()
    {
        ...
        $icon = $price = $button = $title = $ribon = $price_title = $price_pos = $icon = '';
        $css = '';
        if ($stylefiles[18] != '') {
            $price_title = '
        ...
        .oxi-addons-wrapper-' . $oxiid . '  .oxi-icons{
            ...
            line-height:1;
            color: ' . $styledata[453] . ';
            background: ' . $styledata[455] . ';
            border:  ' . $styledata[457] . 'px ' . $styledata[458] . ';
            border-color: ' . $styledata[461] . ';
            border-radius: ' . OxiAddonsPaddingMarginSanitize($styledata, 479) . ';
            ' . OxiAddonsBoxShadowSanitize($styledata, 495) . '
            padding: ' . OxiAddonsPaddingMarginSanitize($styledata, 463) . ';   
            width: ' . $styledata[441] . 'px;
            height: ' . $styledata[445] . 'px;   
            z-index: 3;
            overflow: hidden;
        }
        ...
        .oxi-addons-wrapper-' . $oxiid . '  .oxi-icons{ 
            ...
            padding: ' . OxiAddonsPaddingMarginSanitize($styledata, 464) . ';   
            width: ' . $styledata[442] . 'px;
            height: ' . $styledata[446] . 'px;    
        }
        }
        @media only screen and (max-width : 668px){
            .oxi-addons-wrapper-' . $oxiid . '{ 
